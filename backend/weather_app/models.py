...
class Weather(models.Model):
    ...
    # 温度字段：保留原始带°的字符串格式（与数据库一致）
    max_temp = models.CharField(
        max_length=10,
        verbose_name="最高气温",
        help_text="带°的温度，例如：35°",
        default="25°"
    )
    ...
    min_temp = models.CharField(
        max_length=10,
        verbose_name="最低气温",
        help_text="带°的温度，例如：20°",
        default="15°"
    )
    
    # 天气状况：与原始数据匹配（支持"多云~晴"等复合状态）
    weather = models.CharField(
        max_length=50,  # 加长长度以支持"多云~晴"等格式
        verbose_name="天气状况",
        default="晴"
    )
    
    # 风力风向：保留原始字段（与数据库的wind字段对应）
    ...
    # AQI：保留原始混合格式（如"151 中度"）
    aqi = models.CharField(
        max_length=20, 
        verbose_name="空气质量指数",
        default="100 (良)"
    )
    ...
    def __str__(self):
        return f"{self.city} {self.date} 的天气"